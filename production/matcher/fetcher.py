...
Most country/doc_type combos never clear MIN_REFS from id_alpha/ alone
(passports are almost always a single front-page sample) and most have no
matching Wikimedia category at all, so without a negative cache every
verification request for one of these would re-hit the Wikimedia API and
fail the same way, forever. NO_CANDIDATES_TTL bounds that: once a search
comes up empty, skip re-querying for a while instead of retrying on every
request — but still eventually retry, in case Commons gains a category.
...
MAX_SAMPLES = 10
MIN_REFS    = 3
NO_CANDIDATES_MARKER = '.no_wikimedia_candidates'
NO_CANDIDATES_TTL    = 14 * 86400  # re-check Commons every 14 days, not every request

# country/doc_type are joined straight into REFS_ROOT/REFS_ALPHA_ROOT paths
# below — callers are expected to validate at the API boundary (see
# production/api/routers/verify.py), but guard here too so this module is
# never the thing standing between an unvalidated value and a path traversal.
...
def _code(country) -> str:
    if isinstance(country, dict):
        raw = (country.get('code2') or country.get('code') or 'xx').lower()
    else:
        raw = str(country).lower()
...
def _fetch_wikimedia(country_name: str, doc_type: str, code: str, limit: int) -> int:
    ...
    marker = dest / NO_CANDIDATES_MARKER
    if marker.exists():
        age = time.time() - marker.stat().st_mtime
        if age < NO_CANDIDATES_TTL:
            print(f'  [FETCH] skipping {country_name} / {doc_type} — no candidates as of '
                  f'{int(age / 86400)}d ago, retry in {int((NO_CANDIDATES_TTL - age) / 86400)}d')
            return 0
    ...
    if saved == 0:
        # Candidates existed and resolved, but none survived download/validation
        # (e.g. every hit is a sepia historical scan) — retrying immediately would
        # fail the same way, so treat this the same as "no candidates" for the TTL.
        marker.touch()
